...
public class Compat {
	...
	public static void compatify()
	{
		...
		if(pb)
		{
			try
			{
				if(PlayerBeacons.config.enableThaumcraft)
				{
					...
					IArcaneRecipe soul_cap = ThaumcraftApi.addArcaneCraftingRecipe("CAP_soul", new ItemStack(ForbiddenItems.wandCap, 1, 2), (new AspectList()).add(Aspect.ENTROPY, 20).add(Aspect.FIRE, 5).add(Aspect.WATER, 5).add(Aspect.AIR, 5).add(Aspect.EARTH, 5), new Object[]{"NXN", "N N", Character.valueOf('N'), Items.ender_pearl, Character.valueOf('X'), PlayerBeacons.crystalItem});
		...
		if(totes)
		{
			try
			{
				...
				IArcaneRecipe totem_rod = ThaumcraftApi.addArcaneCraftingRecipe("ROD_totem", new ItemStack(ForbiddenItems.wandCore, 1, 5), (new AspectList()).add(Aspect.ENTROPY, 12).add(Aspect.FIRE, 12).add(Aspect.WATER, 12).add(Aspect.AIR, 12).add(Aspect.EARTH, 12).add(Aspect.ORDER, 12), new Object[]{"PF", "FP", Character.valueOf('F'), new ItemStack(subItems, 1, 2)});
	...
	private static void aspectBloodBlock(String target, int damage, AspectList list)
	{
		try
		{
			...
			ThaumcraftApi.registerObjectTag(new ItemStack(block, damage), list);
		}
		catch(Exception e)
		{
			FMLLog.log(Level.INFO, e, "Forbidden Magic was unable to add aspects to " + target);
